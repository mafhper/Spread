document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchYouTubeMusicData(url) {
        try {
            ...
            console.log('Não foi possível buscar dados específicos do YouTube Music:', error);
            return null;
        }
    }
    ...
    async function parseLink(url) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error('Não foi possível buscar os dados do link. Status: ' + response.status);
            }
            ...
            const base64Image = imageUrl ? await toBase64(imageUrl, 'image/jpeg') : null;
            const finalImageSrc = base64Image || fallbackImageSrc;
            
            const faviconUrl = data.logo?.url || `https://www.google.com/s2/favicons?domain=${new URL(originalInputUrl).hostname}&sz=64`;
            const base64Favicon = faviconUrl ? await toBase64(faviconUrl, 'image/png') : null;
            ...
            if (error.name === 'AbortError') {
                errorMsg = 'Tempo esgotado. A API demorou muito para responder. Tente novamente.';
            } else {
    ...
    function getPlatformIcon(domain) {
        ...
        if (domain.includes('music.apple.com') || domain.includes('apple.com/music')) {
    ...
    function updatePreviewUI(data) {
        ...
        if (data.image && data.image !== '') {
            ...
            if (!previewImage.src.startsWith('data:')) {
                console.error("ERRO: Imagem principal não é Base64. A conversão falhou.");
    ...
    function applyTemplate(data) {
        ...
        if (domain.includes('music.youtube.com') || domain.includes('spotify.com') || domain.includes('bandcamp.com')) {
            ...
            } else if (data.author && data.author !== 'YouTube' && data.author !== 'YouTube Music') {
    ...
    function showDownloadStatus(message, type = 'info') {
        ...
        downloadStatus.className = 'text-sm text-center min-h-[20px] message-' + type;
    }
    ...
    function updateImageStyle() {
        ...
        const positionClasses = ['object-center', 'object-top', 'object-bottom', 'object-left', 'object-right'];
    ...
    function updateFontSize() {
        ...
        const titles = [templateData.defaultTitle, templateData.musicTitle, templateData.newsHeadline];
        const descriptions = [templateData.defaultDescription, templateData.musicArtist, templateData.newsAuthor, templateData.newsDescription];
    ...
    function handleImageError() {
        console.error('Falha catastrófica ao carregar imagem (mesmo Base64). Usando fallback.');
        previewImage.src = fallbackImageSrc; 
        dominantColor = null; 
        setAssistButtonsDisabled(true); 
        showLoading(false);
    ...
    async function downloadImage() {
        ...
        try {
            ...
            showDownloadStatus(`Falha ao gerar o download: ${error.message || 'Erro desconhecido'}`, 'error');
        }
    }
    ...
    assistButtons.analogous.addEventListener('click', () => applyColorAssist('analogous'));
    assistButtons.triad.addEventListener('click', () => applyColorAssist('triad'));
    assistButtons.split.addEventListener('click', () => applyColorAssist('split'));
    ...
    previewFavicon.onerror = () => { previewFavicon.src = fallbackFaviconSrc; };
    ...
    applyTemplate({ 
        ...
        description: 'Crie e compartilhe visualizações de links elegantes e modernas, 100% no seu navegador.',
        domain: 'Spread',
        author: null
    });
    updateGradient(); 
